...
class Converter(ConverterBase):
    ...
    def config_entities(cls):
        ...
        yield OptionalConfigEntity(
            "field_name", None, "Key name to be used as field name in TSDB."
        )
        yield OptionalConfigEntity(
            "field_value", "value", "Key name to be used as value in TSDB."
        )

    def convert(self, payload):
        p = Point.measurement(self.config.measurement).time(
            int(payload["timestamp"] * 1e9), write_precision=WritePrecision.NS
        )
